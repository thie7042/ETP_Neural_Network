...
for _ in range(300):
   ...
   # ridge = Ridge(alpha = 0.001, normalize = True)
    ...
    ridge = Ridge(alpha = 0.059, normalize = True)
    ...
    if acc > best:
        best = acc
        #store model
        with open("studentmodelridge.pickle", "wb") as f:
            pickle.dump(ridge,f)
...
pickle_in = open("studentmodelridge.pickle", "rb")
...
######################################
'''data = pd.read_csv("Masterdata.csv")
second_predictions = ridge.predict(X)
